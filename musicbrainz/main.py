...
def search(artist: str, search_title: str) -> Song:
    try:
        with psycopg2.connect(
            ...
            with conn.cursor() as cursor:
                ...
                where = """"artist"."name" LIKE '%{}%'""".format(artist.replace("'", "''"))
...
with open('mbdata.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        song = search(artist_name, title)

        artist_db = "{} {} ({})".format(row["artist_musicbrainz_id"], row["artist_name"], row["artist_country_id"])
        print("DB: {}".format(artist_db))
        if song is not None:
            artist_mb = "{} {} ({})".format(song.recording_artist_id, song.recording_artist, song.country_id)
        ...
        if song is not None:
            album_title = song.release_group_name
